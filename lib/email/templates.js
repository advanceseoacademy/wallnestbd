...
function paymentLabel(method) {
  const map = { bkash: 'bKash', rocket: 'Rocket', nagad: 'Nagad', cod: 'Cash on Delivery' };
...
function buildNewOrderOwnerEmail({
  orderNumber,
  items,
  subtotal,
  shipping,
  ...
  const home = siteUrl || process.env.BASE_URL || 'http://localhost:3000';
  const adminUrl = `${home}/admin/orders`;
  const address = [shippingAddress, shippingCity].filter(Boolean).join(', ');
  const areaLabel = deliveryArea === 'outside_dhaka' ? 'ঢাকার বাইরে' : 'ঢাকার ভিতরে';
  ...
  const itemRows = (items || [])
    .map(
      (item) => `<tr>
        ...
        <td style="padding:10px;border-bottom:1px solid #F3F4F6;font-size:14px;text-align:center;">${item.quantity || item.qty || 1}</td>
        <td style="padding:10px;border-bottom:1px solid #F3F4F6;font-size:14px;text-align:right;font-weight:600;">${formatMoney(item.line_total ?? item.price * (item.quantity || item.qty || 1))}</td>
      </tr>`
    )
    .join('');

  const row = (label, value) => `<tr>
      <td style="padding:8px 10px;font-size:13px;color:${MUTED};white-space:nowrap;">${escapeHtml(label)}</td>
      <td style="padding:8px 10px;font-size:14px;font-weight:600;">${value}</td>
    </tr>`;

  const bodyHtml = `
    <div style="text-align:center;margin-bottom:8px;">
      <span style="display:inline-block;background:#FEF3C7;color:#92400E;font-size:12px;font-weight:700;padding:6px 14px;border-radius:20px;">নতুন অর্ডার / New Order</span>
    </div>
    <h1 style="margin:16px 0 8px;font-size:22px;font-weight:800;color:${TEXT};text-align:center;">
      🔔 নতুন অর্ডার এসেছে — ${formatMoney(total)}
    </h1>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:12px;margin:20px 0;">
      <tr>
    ...
    <h2 style="font-size:16px;font-weight:700;margin:0 0 8px;color:${TEXT};">কাস্টমার</h2>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #E5E7EB;border-radius:10px;overflow:hidden;margin-bottom:22px;">
      ${row('নাম', escapeHtml(customerName || '—'))}
      ${row('ফোন', customerPhone ? `<a href="tel:${escapeHtml(customerPhone)}" style="color:${PRIMARY};text-decoration:none;">${escapeHtml(customerPhone)}</a>` : '—')}
      ${row('ঠিকানা', escapeHtml(address || '—'))}
      ${row('ডেলিভারি এরিয়া', escapeHtml(areaLabel))}
      ${customerEmail ? row('ইমেইল', escapeHtml(customerEmail)) : ''}
      ${customerNote ? row('নোট', escapeHtml(customerNote)) : ''}
      ${row('পেমেন্ট', `${escapeHtml(paymentLabel(paymentMethod))}${isCod ? ' <span style="color:#B45309;">(ক্যাশ কালেক্ট করতে হবে)</span>' : ''}`)}
    </table>
    <h2 style="font-size:16px;font-weight:700;margin:0 0 12px;color:${TEXT};">পণ্য</h2>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #E5E7EB;border-radius:10px;overflow:hidden;">
      <tr style="background:#F9FAFB;">
        <th style="padding:10px;text-align:left;font-size:12px;color:${MUTED};">পণ্য</th>
        <th style="padding:10px;text-align:center;font-size:12px;color:${MUTED};">পরিমাণ</th>
        <th style="padding:10px;text-align:right;font-size:12px;color:${MUTED};">মূল্য</th>
      </tr>
      ${itemRows}
      <tr>
        <td colspan="2" style="padding:10px;font-size:14px;color:${MUTED};text-align:right;">সাবটোটাল</td>
        <td style="padding:10px;font-size:14px;text-align:right;">${formatMoney(subtotal)}</td>
      </tr>
      <tr>
        <td colspan="2" style="padding:10px;font-size:14px;color:${MUTED};text-align:right;">ডেলিভারি চার্জ</td>
        <td style="padding:10px;font-size:14px;text-align:right;">${shipping > 0 ? formatMoney(shipping) : 'ফ্রি'}</td>
      </tr>
      <tr style="background:#EFF6FF;">
        <td colspan="2" style="padding:14px 10px;font-size:15px;font-weight:700;text-align:right;">মোট</td>
        <td style="padding:14px 10px;font-size:18px;font-weight:800;color:${PRIMARY};text-align:right;">${formatMoney(total)}</td>
    ...
    ${ctaButton(adminUrl, '⚙️ অ্যাডমিন প্যানেলে দেখুন')}
    <p style="margin:24px 0 0;font-size:13px;line-height:1.6;color:${MUTED};text-align:center;">
      কাস্টমারকে কল করে অর্ডার কনফার্ম করুন, তারপর অ্যাডমিন প্যানেল থেকে স্ট্যাটাস আপডেট করুন।
    </p>
  `;

  const textLines = [
    `নতুন অর্ডার: ${orderNumber}`,
    `মোট: ${formatMoney(total)} (${paymentLabel(paymentMethod)})`,
    `নাম: ${customerName || '—'}`,
    `ফোন: ${customerPhone || '—'}`,
    `ঠিকানা: ${address || '—'} (${areaLabel})`,
    customerNote ? `নোট: ${customerNote}` : null,
    `অ্যাডমিন: ${adminUrl}`,
  ].filter(Boolean);

  return {
    subject: `🔔 নতুন অর্ডার ${orderNumber} — ${formatMoney(total)} · ${customerPhone || 'ফোন নেই'}`,
    html: baseLayout({
      title: `New order ${orderNumber}`,
      preheader: `${customerName || 'গ্রাহক'} — ${formatMoney(total)} — ${customerPhone || ''}`,
      bodyHtml,
      siteUrl: home,
    }),
    text: textLines.join('\n'),
