...
const { getStoreSettings, computeShipping, normalizeDeliveryArea } = require('./storeSettings');
...
const { queueConsignmentForOrder } = require('./steadfast');
const {
...
async function handleApiRequest(method, segments, req, { query = {}, body = {} } = {}) {
  ...
  try {
    ...
    if (method === 'POST' && path === 'cart/clear') {
      await clearCart(req);
      return json(200, { ok: true, items: [], count: 0, subtotal: 0 });
    }
    ...
    if (method === 'POST' && path === 'orders') {
      ...
      if (!delivery_area || !['inside_dhaka', 'outside_dhaka'].includes(delivery_area)) {
        return json(400, { error: 'Delivery area is required' });
      }
      if (!payment_method || !['bkash', 'rocket', 'nagad', 'cod'].includes(payment_method)) {
        return json(400, { error: 'Valid payment method required' });
      }
