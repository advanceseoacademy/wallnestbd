  ...
  try {
    ...
    if (method === 'GET' && path === 'payments') {
      const { data } = await supabase
        ...
        .maybeSingle();
      return json(200, { payments: data?.value || {} });
    }
    ...
    if (method === 'POST' && path === 'auth/register') {
      ...
      return json(200, {
        ok: true,
        message: 'Check email to confirm account (if enabled)',
      });
    }
