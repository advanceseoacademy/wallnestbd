...
function getAdminDb() {
  try {
    return require('./supabaseAdmin').getSupabaseAdmin();
  } catch {
    return supabase;
  }
}
...
async function updateCategory(id, body) {
  const { data: current, error: fetchErr } = await getAdminDb()
    .from('categories')
    .select('*')
    .eq('id', id)
    .single();
  ...
  if (slug !== current.slug) {
    const { data: dup } = await getAdminDb()
      .from('categories')
      .select('id')
      .eq('slug', slug)
      .neq('id', id)
...
async function deleteCategory(id) {
  ...
  const { error } = await getAdminDb().from('categories').delete().eq('id', id);
  if (error) throw error;
  bustCatalog();
  return { ok: true };
}
...
async function updatePaymentSettings(payments) {
  const { error } = await getAdminDb().from('site_settings').upsert({
    key: 'payment_methods',
    value: payments,
