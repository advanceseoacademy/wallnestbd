...
async function getOrders() {
  ...
  // Refresh stale courier statuses so the table reflects the courier, not our cache.
  const freshCourierStatus = await steadfast.syncPendingOrders(data || []);

  return {
    courierConfigured: steadfast.configured(),
    orders: (data || []).map((o) => ({
      id: o.id,
      ...
      deliveryAreaLabel:
        o.delivery_area === 'outside_dhaka'
          ? 'ঢাকার বাইরে'
          : o.delivery_area === 'inside_dhaka'
            ? 'ঢাকার ভিতরে'
            : '',
      items: o.order_items || [],
      firstProduct: o.order_items?.[0],
      courierConsignmentId: o.courier_consignment_id || null,
      courierTrackingCode: o.courier_tracking_code || '',
      courierStatus: freshCourierStatus[o.id] || o.courier_status || '',
      courierStatusLabel: steadfast.statusLabel(freshCourierStatus[o.id] || o.courier_status),
      courierError: o.courier_error || '',
    })),
  };
}

async function sendOrderToCourier(id) {
  const db = await getAdminDb();
  ...
  const result = await steadfast.createConsignmentForOrder(order, order.order_items);
  if (!result.ok) throw new Error(result.error);
  return { ok: true, ...result };
}

async function syncOrderCourierStatus(id) {
  const db = await getAdminDb();
  const { data: order, error } = await db.from('orders').select('*').eq('id', id).maybeSingle();
  if (error) throw error;
  if (!order) throw new Error('অর্ডার পাওয়া যায়নি');
  if (!order.courier_consignment_id) throw new Error('এই অর্ডার এখনো কুরিয়ারে পাঠানো হয়নি');

  const courierStatus = await steadfast.syncOrderStatus(order, { force: true });
  return { ok: true, courierStatus, courierStatusLabel: steadfast.statusLabel(courierStatus) };
}

async function getCourierBalance() {
  if (!steadfast.configured()) throw new Error('Steadfast API কনফিগার করা হয়নি');
  return steadfast.getBalance();
}
...
async function deleteOrder(id) {
  ...
  if (!existing) throw new Error('অর্ডার পাওয়া যায়নি');
...
async function getShippingSettings() {
  const { data } = await supabase
    ...
    .maybeSingle();
  const ship = data?.value || {};
  const legacyFee = Number(ship.shipping_fee ?? 80);
  return {
    shipping: {
      free_shipping_min: Number(ship.free_shipping_min ?? ship.min_order ?? 1500),
      shipping_fee: legacyFee,
      inside_dhaka_fee: Number(ship.inside_dhaka_fee ?? legacyFee ?? 60),
      outside_dhaka_fee: Number(ship.outside_dhaka_fee ?? legacyFee ?? 120),
    },
  };
}

async function updateShippingSettings(shipping) {
  const payload = {
    free_shipping_min: Number(shipping.free_shipping_min ?? 1500),
    shipping_fee: Number(shipping.inside_dhaka_fee ?? shipping.shipping_fee ?? 80),
    inside_dhaka_fee: Number(shipping.inside_dhaka_fee ?? 60),
    outside_dhaka_fee: Number(shipping.outside_dhaka_fee ?? 120),
  };
  const { error } = await (await getAdminDb()).from('site_settings').upsert({
    key: 'shipping_rules',
    value: payload,
  ...
  if (error) {
    await adminQuery(
      `UPDATE site_settings SET value = $1::jsonb, updated_at = NOW() WHERE key = 'shipping_rules'`,
      [JSON.stringify(payload)]
    );
  }
  const { bustStoreSettingsCache } = require('./storeSettings');
  bustStoreSettingsCache();
  return { ok: true, shipping: payload };
}
