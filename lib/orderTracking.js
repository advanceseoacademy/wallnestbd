...
function mapOrderForTracking(order) {
  ...
  return {
    ...
    courierStatusLabel: steadfast.statusLabel(order.courier_status),
...
async function trackOrderByNumber(orderNumber) {
  ...
  // Pull a fresh courier status when the cached one is stale, so the customer
  // sees where the parcel actually is rather than our last known state.
  const courierStatus = await steadfast.syncOrderStatus(order);
  if (courierStatus && courierStatus !== order.courier_status) {
    order.courier_status = courierStatus;
    const mapped = steadfast.mapCourierStatusToOrderStatus(courierStatus);
    if (mapped && order.status !== 'cancelled') order.status = mapped;
  }
