...
function seoForMotionSensorOffer(baseUrl, product = null) {
  const image =
    product?.imageUrl || '/images/e27-pir-motion-sensor-hero.jpg';
  const price = product?.price != null ? Number(product.price) : 450;
  return buildSeo({
    title: 'স্মার্ট মোশন সেন্সর LED বাল্ব — অন্ধকারে নিজে জ্বলে',
    description:
      'PIR + লাইট সেন্সর LED বাল্ব ১৫W — অন্ধকারে পা রাখামাত্র জ্বলে, কেউ না থাকলে নিভে যায়। ক্যাশ অন ডেলিভারি।',
    canonical: `${baseUrl}/offer/motion-sensor-light`,
    image,
    type: 'product',
    jsonLd: [
      breadcrumbJsonLd([
        { name: 'হোম', url: `${baseUrl}/` },
        { name: 'মোশন সেন্সর লাইট', url: `${baseUrl}/offer/motion-sensor-light` },
      ]),
      {
        '@context': 'https://schema.org',
        '@type': 'Product',
        name: product?.nameBn || product?.name || 'E27 PIR Motion Sensor LED Bulb 15W',
        description:
          'E27 PIR + light sensor LED bulb 15W with auto on/off — stairs, corridors and garages.',
        ...
        offers: {
          ...
          url: `${baseUrl}/offer/motion-sensor-light`,
        },
      },
    ],
  });
