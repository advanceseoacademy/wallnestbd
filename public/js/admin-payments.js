(function () {
const keys = ['bkash', 'rocket', 'nagad', 'cod'];
const labels = { bkash: 'bKash', rocket: 'Rocket', nagad: 'Nagad', cod: 'Cash on Delivery' };
  ...
  form.innerHTML = keys
    .map((key) => {
      ...
      <div class="card" style="margin-bottom:16px;padding:20px;background:var(--surface2);border:1px solid var(--border);border-radius:12px;">
        ...
        ${isCod ? '' : `<div class="form-group"><label class="form-label">নম্বর</label><input class="form-input" data-key="${key}" data-field="number" value="${p.number || ''}"></div>
